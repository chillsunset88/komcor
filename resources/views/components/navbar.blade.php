...
<header class="sticky top-0 z-50 border-b border-slate-200/70 bg-white/75 backdrop-blur supports-[backdrop-filter]:bg-white/60 dark:border-slate-800/70 dark:bg-slate-900/70">
    <div class="mx-auto flex max-w-6xl flex-wrap items-center gap-3 px-4 py-4 sm:px-6 lg:px-8">
        <div class="flex items-center gap-3">
            <a href="/" class="flex items-center gap-3 text-slate-900 dark:text-white">
                ...
                <div class="flex flex-col leading-tight">
                    <span class="text-lg font-semibold">Kompas Corner</span>
        ...
        <nav class="order-4 hidden w-full items-center justify-center gap-6 text-sm font-semibold text-slate-600 lg:order-none lg:flex lg:w-auto lg:flex-1 dark:text-slate-300">
            @foreach($menus as $m)
        ...
        <form action="{{ route('search') }}" method="GET" class="order-3 flex w-full items-center gap-2 rounded-full border border-slate-200/80 bg-white px-4 py-2 text-sm shadow-sm transition focus-within:border-slate-900 md:order-none md:w-auto dark:border-white/10 dark:bg-white/10">
        ...
        <div class="order-2 ml-auto flex w-full flex-wrap items-center justify-end gap-2 md:order-none md:w-auto md:flex-nowrap">
            <button type="button" onclick="toggleTheme()" class="inline-flex size-9 items-center justify-center rounded-full border border-slate-200 text-slate-600 transition hover:border-slate-900 hover:text-slate-900 dark:border-white/10 dark:text-white dark:hover:border-white/40">
            ...
            @auth
                <div x-data="{ open: false }" class="relative flex w-full justify-end md:w-auto">
                    ...
                    <div x-cloak x-show="open" x-transition @click.outside="open = false" class="absolute right-0 top-full mt-3 w-72 rounded-3xl border border-slate-200/80 bg-white/95 p-4 shadow-xl dark:border-white/10 dark:bg-slate-900/95">
                        ...
                        <div class="space-y-2 text-sm">
                            ...
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button class="flex w-full items-center justify-between rounded-2xl border border-red-200 px-4 py-2 font-semibold text-red-600 transition hover:border-red-400 hover:text-red-700 dark:border-red-500/30 dark:text-red-300 dark:hover:border-red-400">Keluar<span class="text-xs">?</span></button>
                            </form>
                        </div>
                    </div>
                </div>
            @else
                <a href="{{ route('login') }}" class="flex w-full items-center justify-center rounded-full border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 transition hover:border-slate-900 hover:text-slate-900 md:w-auto dark:border-white/10 dark:text-white">Masuk</a>
            @endauth
        </div>
    </div>
        ...
        <div class="space-y-6 border-t border-slate-200 bg-white px-4 pb-8 pt-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <div class="space-y-4">
                ...
                <div class="grid gap-3">
                    @foreach($menus as $m)
                        <a href="{{ $m->url }}" class="rounded-2xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-600 transition hover:border-slate-900 hover:text-slate-900 dark:border-white/10 dark:text-slate-200">{{ $m->title }}</a>
                    @endforeach
                    @unless($hasAbout)
                        <a href="{{ route('about') }}" class="rounded-2xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-600 transition hover:border-slate-900 hover:text-slate-900 dark:border-white/10 dark:text-slate-200">About</a>
                    @endunless
                </div>
            </div>
            ...
            @auth
                <div class="space-y-4 rounded-3xl border border-slate-200 px-4 py-4 dark:border-white/10">
                    ...
                    <a href="{{ route('dashboard') }}" class="block rounded-2xl border border-slate-200 px-4 py-2 text-center text-sm font-semibold text-slate-600 transition hover:border-slate-900 hover:text-slate-900 dark:border-white/10 dark:text-slate-200">Pengaturan Akun</a>
                    @if(auth()->user()->isAdmin())
                        <a href="{{ route('admin.dashboard') }}" class="block rounded-2xl border border-slate-200 px-4 py-2 text-center text-sm font-semibold text-slate-600 transition hover:border-slate-900 hover:text-slate-900 dark:border-white/10 dark:text-slate-200">Dashboard Admin</a>
                    @elseif(auth()->user()->isAuthor())
                        <a href="{{ route('posts.index') }}" class="block rounded-2xl border border-slate-200 px-4 py-2 text-center text-sm font-semibold text-slate-600 transition hover:border-slate-900 hover:text-slate-900 dark:border-white/10 dark:text-slate-200">Dashboard Author</a>
                    @endif
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
            ...
            @else
                <a href="{{ route('login') }}" class="flex w-full items-center justify-center rounded-2xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-600 transition hover:border-slate-900 hover:text-slate-900 dark:border-white/10 dark:text-slate-200">Masuk / Daftar</a>
            @endauth
        </div>
    </div>
</header>
